...
class _SignInState extends State<SignIn> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<AuthBloc, AuthStates>(
      listener: (_, state) {
        if (state is LoginSuccessState) {
          ...
          if (state.user.Role == "2") {
            //if role is ADMIN go to ADMIN screen
            ...
            //If role is TECHNICIAN go to admin screen
            Navigator.of(context).pushReplacementNamed(
                Technician_main.routeName,
                arguments: UserArgument(user: user));
          } else {
            //else go to USER Screen
            Navigator.of(context).pushReplacementNamed(Users_main.routeName,
                arguments: UserArgument(user: user));
          }
        } else if (state is LoginFailedState) {
          Toast.show("Incorrect Email/Password combination", context,
              duration: Toast.LENGTH_SHORT, gravity: Toast.BOTTOM);
        }

        if (state is LoggingState) {
          //On loading call the loading event manager UI
          loading_screen(title: "Login loading");
        }
        if (state is UserOperationFailure) {
          //Toast the message of the Failure
          Toast.show("Login operation failed :(", context,
      ...
      builder: (_, state) {
        //Build the page if not logging in
        return Scaffold(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            gradientbutton(
                                state), //Our custom Widget, takes the current state
  ...
  Widget gradientbutton(AuthStates state) {
    //Custom widget
