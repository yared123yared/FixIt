...
class ServiceDataProvider {
  ...
  Future<Service> createService(Service service) async {
    final response = await httpClient.post(
      Uri.http('192.168.137.1:4000', '/Service'),
...
//    Response response = await get(Uri.encodeFull('http://192.168.137.1:3000/Service'));
    try {
//     print("try catch method");
      final response = await http.get('http://192.168.137.1:4000/Service');
      if (response.statusCode == 200) {
...
//       print("geting service from remote server failed");
  ...
  Future<void> deleteService(int id) async {
    ...
    final http.Response response = await http.delete(
      'http://192.168.137.1:4000/Service/$id',
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );
  ...
  Future<void> updateService(Service service) async {
    final http.Response response = await httpClient.put(
      'http://192.168.137.1:4000/Service/',
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(<String, dynamic>{
